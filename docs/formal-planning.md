...
Plan Auditor can optionally attach an **LLM-free classical planning proof** to a
normal plan. This extends the existing L5 structural verifier with explicit
symbolic state, preconditions, add effects, delete effects, and final goals.

The design uses two independent mechanisms:

1. an in-process grounded STRIPS-style planner that is always available and uses
   only Python/CPU, and
2. optional PDDL export + Fast Downward execution for an external classical
   planner cross-check.
...
A canonical anchor looks like this:
...
plan graph. The deterministic audit then independently proves the concrete work.

## Trust boundary

Formal planning cannot repair an incorrect formalization. If a user requirement
is translated into the wrong initial facts, preconditions, effects, or goals, a
planner can prove the wrong model correctly. For high-assurance work, the formal
contract itself should therefore be reviewed or generated from a host-approved
