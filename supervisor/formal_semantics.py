...
def _requirements(plan: Mapping[str, Any]) -> List[Dict[str, str]]:
    ...
    for index, item in enumerate(raw, 1):
        if isinstance(item, str):
            result.append({"id": f"REQ-{index:03d}", "description": item, "priority": "must"})
            continue
...
def analyze_formal_semantics(plan: Mapping[str, Any]) -> FormalSemanticResult:
    ...
    if len(anchors) != 1:
        result.valid = False
        result.errors.append(f"formal semantic alignment requires exactly one formal_planning anchor; found {len(anchors)}")
        return result

    contract = anchors[0].get(FORMAL_KEY)
    if not isinstance(contract, Mapping):
        result.valid = False
        result.errors.append("formal_planning must be an object for semantic alignment")
        return result

    goals_raw = contract.get("goal_facts", [])
    initial_raw = contract.get("initial_facts", [])
    actions_raw = contract.get("actions", [])
    goals = {x for x in goals_raw if isinstance(x, str)} if isinstance(goals_raw, list) else set()
    initial = {x for x in initial_raw if isinstance(x, str)} if isinstance(initial_raw, list) else set()
    ...
    for step in plan.get("steps", []) or []:
        ...
        step_covers[int(step["id"])] = {
            x.strip() for x in covers if isinstance(x, str) and x.strip()
        } if isinstance(covers, list) else set()
    ...
    if isinstance(actions_raw, list):
        for action in actions_raw:
            if not isinstance(action, Mapping) or not isinstance(action.get("step"), int):
                continue
            sid = int(action["step"])
            adds = {x for x in action.get("add_effects", []) if isinstance(x, str)} if isinstance(action.get("add_effects", []), list) else set()
            deletes = {x for x in action.get("del_effects", []) if isinstance(x, str)} if isinstance(action.get("del_effects", []), list) else set()
    ...
    for sid in sorted(effect_free):
        result.errors.append(
            f"formal action for step {sid} has no symbolic effect; every plan step must participate in the formal model"
        )
    ...
    for req in _requirements(plan):
        ...
        if fact in initial:
            result.errors.append(
                f"required requirement {req_id} formal goal fact is already true initially; it must be produced by verified work"
            )
        ...
        if not fact_producers:
            ...
            continue
        bad = [sid for sid in fact_producers if req_id not in step_covers.get(sid, set())]
        if bad:
            result.errors.append(
                f"requirement {req_id} goal fact is produced by non-covering step(s): {bad}"
            )
        if not any(req_id in step_covers.get(sid, set()) for sid in fact_producers):
            result.errors.append(
                f"requirement {req_id} goal fact is not produced by any step that covers the requirement"
            )
    ...
    if unbound:
        result.warnings.append(
            "formal goal fact(s) are not direct must/should requirement bindings: " + ", ".join(unbound)
        )
